...
import Browser
import Html exposing (Html, text, h1, input, button, p, hr, div, a, span, pre, code)
import Html.Attributes exposing (class, type_, placeholder, value, id, href)
...
update msg model =
    case msg of
        EnteredBits str ->
            let
                oldKeyCriation = model.keyCreation
                newKeyCreation = { oldKeyCriation | bits = Maybe.withDefault model.keyCreation.bits (String.toInt str) }
            in
                ({model | keyCreation = newKeyCreation}, Cmd.none)
            
        AddCreationPassword str->
            let
                keyCreation = model.keyCreation
                newKeyCreation = { keyCreation | password = str }
            in
                ({ model | keyCreation = newKeyCreation}, Cmd.none)
        ...
        AddCreationIdentifier str->
            let
                keyCreation = model.keyCreation
                newKeyCreation = { keyCreation | identifier = str }
            in
                ({ model | keyCreation = newKeyCreation}, Cmd.none)
        ...
        PrivKeyCreated privkey ->
            let
                newKey = PrivKey privkey model.keyCreation.password False
            in
                ({ model | privKey = newKey } , Cmd.none)

        EnteredPrivKey str ->
            let
                newprivKey = PrivKey (RemoteData.Success str) model.privKey.gpgPrivKeyPass model.privKey.gpgPrivSaveToDisk
            in
                ({ model | privKey = newprivKey } , Cmd.none)

        EnteredPrivKeyPass str ->
            let
                newprivKey = PrivKey model.privKey.gpgPrivKey str model.privKey.gpgPrivSaveToDisk
            in
                ({ model | privKey = newprivKey } , Cmd.none)
        ...
        SubmitKey ->
            case model.privKey.gpgPrivKey of
                RemoteData.Success key ->
                    let
                        newKey = ValidPrivKey key model.privKey.gpgPrivKeyPass True
                    in
        ...
        UpdateFingerprint str ->
            let
                oldUnlockInfo = model.unlockInfo
                newUnlockInfo = {oldUnlockInfo | fingerprint = str}
                newModel = { model | unlockInfo = newUnlockInfo }
            in
                (newModel , Cmd.none)
        ...
        GotPubKey resp ->
            case resp of
                RemoteData.Success key ->
                    let
                        oldUnlockInfo = model.unlockInfo
                        newUnlockInfo = { oldUnlockInfo | fingerprint = key.fingerprint, password = model.keyCreation.password }
                    in
        ...
        GotUnlockInfo resp ->
            let
                oldUnlockInfo = model.unlockInfo
                newUnlockInfo = { oldUnlockInfo | unlockStatus = resp}
                newModel = {model | unlockInfo = newUnlockInfo}
            in
                (newModel, Cmd.none)
        ...
        UpdateUnlockPassword str ->
            let
                oldUnlockInfo = model.unlockInfo
                newUnlockInfo = {oldUnlockInfo | password = str}
            in
